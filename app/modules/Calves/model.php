<artifact identifier="calves-model" type="application/vnd.ant.code" language="php" title="Calves Model">
<?php
/**
 * Calves Model
 * Database operations for calf management
 */
class CalvesModel {
    private $db;
    ...
    /**
     * Get all calves (excluding deleted)
     */
     ...
     * Get calf by ID
     */
    public function getCalfById($id) {
    ...
    /**
     * Check if calf ID exists
     */
    public function calfIdExists($calfId, $excludeId = null) {
        $query = "SELECT id FROM calves WHERE calf_id = ?";
        $params = [$calfId];
    ...
    /**
     * Delete calf permanently (with confirmation)
     */
    public function deleteCalf($id) {
        // Delete all related records first
        $this->db->query("DELETE FROM calf_events WHERE calf_id = ?", [$id]);
        $this->db->query("DELETE FROM treatment_plans WHERE calf_id = ?", [$id]);
        $this->db->query("DELETE FROM treatment_completions WHERE calf_id = ?", [$id]);
        
        // Delete the calf
        return $this->db->query("DELETE FROM calves WHERE id = ?", [$id]);
    }

    /**
     * Soft delete calf (mark as deleted)
     */
    public function softDeleteCalf($id) {
    ...
    /**
     * Bulk soft delete calves
     */
    public function bulkSoftDeleteCalves($calfIds) {
        if (empty($calfIds)) {
            return 0;
        }
...
### File 15: Updated Import View - `app/modules/Calves/views/import.php`

<artifact identifier="import-view-updated" type="application/vnd.ant.code" language="php" title="Enhanced Import View">
<?php
require_once BASE_PATH . '/app/core/ModernNavbar.php';
$navbar = new ModernNavbar();

$fileParser = new FileParser();
$supportedFormats = $fileParser->getSupportedFormats();
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div class="import-container">
        ...
        <p style="text-align: center; color: var(--text-secondary); margin-bottom: 2rem;">
            Import calves from CSV, Excel, or text files with intelligent column detection
        </p>
        ...
        <div class="import-section">
            ...
            <div class="info-box">
                ...
                <ul style="margin: 0.5rem 0 0 1.5rem; line-height: 1.8;">
                    ...
                    <li>Multiple format support: CSV, Excel, TSV, TXT</li>
            ...
            <form method="post" enctype="multipart/form-data" id="importForm">
                <label for="file_upload" class="upload-area" id="uploadArea">
                    ...
                    <input type="file" name="import_file" id="file_upload" class="file-input" 
                           accept=".csv,.xlsx,.xls,.txt,.tsv" required>
                </label>
    ...
    <script>
        ...
        fileInput.addEventListener('change', function() {
            if (this.files.length > 0) {
                ...
                // Validate file type
                const validExtensions = <?= json_encode($supportedFormats) ?>;
